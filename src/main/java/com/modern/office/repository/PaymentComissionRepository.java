...
public interface PaymentComissionRepository extends CrudRepository<PaymentComission, Integer> {
    @Query("select p from PaymentComission p where p.provider = :provider and p.paymentDate between :fromDate and :toDate")
    Iterable<PaymentComission> findByProviderAndPaymentDate(String provider, LocalDate fromDate, LocalDate toDate);

    @Query("select p from PaymentComission p where p.provider = :provider and p.insurance in :insurances and p.paymentDate between :fromDate and :toDate")
